...
- Preconditions:
  - Keeper addresses are registered as authorized executors in the CrossChainRegistry on the source
    chain.
  - Adapter peer relationships and Ark ↔ Proxy relationships are configured consistently across
    chains.
...
- Alert on: delivery failures, registry validation failures, pause state changes, abnormal fee
  quotes.
- Reconciliation:
  - Ark: `inflightAssets` updates when transfers are initiated; `lastRemoteAssetBalance` updates on
    message/read responses tied to the latest outgoing transfer.
  - FleetProxy: `latestIncomingTransferId` advances on deposits; `notifySourceChain(...)` can update
    the source with current fleet balance.

#### Failure and Recovery

- Bridge delivery failure: follow the adapter’s documented recovery path (e.g., retrieval by
  governance or retry mechanisms).
- Registry mismatch: verify registry entries on both chains; correct and re-execute only after
  confirmation.
