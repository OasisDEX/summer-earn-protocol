...
export function handleWithdrawExchangeAssetBatch(event: WithdrawExchangeAssetBatch): void {
  ...
  // The event reports the SUM of exchange asset moved across all ids. Attribute
  // the full sum to the first receipt so vault.cumulative... is correct; per-id
  // receipt counters are bumped without re-incrementing the vault aggregate.
