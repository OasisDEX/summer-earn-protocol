...
import { FleetCommanderRewardsManager as FleetCommanderRewardsManagerContract } from '../../generated/InstitutionalVaultRegistry/FleetCommanderRewardsManager'
...
import {
  ...
  RoundsVaultPair as RVRoundsVaultPair,
} from '../../generated/schema'

export function getRoundsVault(vaultId: string): RVRoundsVault | null {
  return RVRoundsVault.load(vaultId)
}

export function getRoundsVaultByAddress(addr: Address): RVRoundsVault | null {
  return RVRoundsVault.load(addr.toHexString())
}

export function getOrCreateRound(
  vault: RVRoundsVault,
  roundId: BigInt,
  block: ethereum.Block,
): RVRound {
  const id = vault.id + '-' + roundId.toString()
  let round = RVRound.load(id)
  if (round != null) {
    return round
  }
  round = new RVRound(id)
  round.vault = vault.id
  round.roundId = roundId
  round.state = 'OPENED'
  round.openedAt = block.timestamp
  round.openedAtBlock = block.number
  round.totalReceiptSupplyAtClose = BigIntConstants.ZERO
...
export function getOrCreateReceipt(
  ...
  const id = vault.id + '-' + round.roundId.toString() + '-' + account.id
  let receipt = RVReceipt.load(id)
  if (receipt != null) {
    return receipt
  }
  receipt = new RVReceipt(id)
  receipt.vault = vault.id
...
export function loadPair(pairId: string): RVRoundsVaultPair | null {
  return RVRoundsVaultPair.load(pairId)
}
