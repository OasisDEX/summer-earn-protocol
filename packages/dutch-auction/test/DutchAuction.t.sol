...
contract DutchAuctionLibraryTest is Test {
    ...
    Percentage public constant KICKER_REWARD_PERCENTAGE = Percentage.wrap(5 * 1e18);
    ...
    function setUp() public {
        ...
        paymentToken.mint(BUYER1, 7_500_000_000_000_000_000_000_000_000_000_000_000_000 ether);
        paymentToken.mint(BUYER2, 7_500_000_000_000_000_000_000_000_000_000_000_000_000 ether);
        paymentToken.mint(BUYER3, 7_500_000_000_000_000_000_000_000_000_000_000_000_000 ether);
    ...
    function testCreateAuctionGuards() public {
        ...
        vm.expectRevert(abi.encodeWithSignature("InvalidKickerRewardPercentage()"));
        vm.prank(AUCTION_KICKER);
        auctionManager.createAuction(
            IERC20(address(auctionToken1)),
            IERC20(address(paymentToken)),
        ...
        (DutchAuctionLibrary.AuctionConfig memory config, DutchAuctionLibrary.AuctionState memory state) =
            auctionManager.auctions(auctionId);
        assertEq(config.id, auctionId, "Auction ID not set correctly");
        assertEq(address(config.auctionToken), address(auctionToken1), "Auction token not set correctly");
        assertEq(address(config.paymentToken), address(paymentToken), "Payment token not set correctly");

        assertEq(config.startPrice, START_PRICE, "Start price not set correctly");
        assertEq(config.endPrice, END_PRICE, "End price not set correctly");
        assertEq(config.totalTokens, TOTAL_TOKENS - KICKER_REWARD_AMOUNT, "Total tokens not set correctly");
        assertEq(
            config.kickerRewardAmount,
            TOTAL_TOKENS.applyPercentage(KICKER_REWARD_PERCENTAGE),
            "Kicker reward percentage not set correctly"
        );
        assertEq(address(config.unsoldTokensRecipient), UNSOLD_RECIPIENT, "Unsold tokens recipient not set correctly");
        assertEq(config.startTime, block.timestamp, "Start time not set correctly");
        assertEq(config.endTime, block.timestamp + AUCTION_DURATION, "End time not set correctly");
        assertEq(state.remainingTokens, TOTAL_TOKENS - KICKER_REWARD_AMOUNT, "Remaining tokens not set correctly");
        assertEq(state.isFinalized, false, "Auction should not be finalized");

        auctionToken1.mint(address(this), 100 ether);

        DutchAuctionLibrary.Auction memory auction = DutchAuctionLibrary.createAuction(
        ...
        assertEq(address(auction.config.auctionToken), address(auctionToken1), "Auction token not set correctly");
        assertEq(address(auction.config.paymentToken), address(paymentToken), "Payment token not set correctly");
        assertEq(auction.config.startPrice, START_PRICE, "Start price not set correctly");
        assertEq(auction.config.endPrice, END_PRICE, "End price not set correctly");
        assertEq(auction.config.totalTokens, TOTAL_TOKENS - KICKER_REWARD_AMOUNT, "Total tokens not set correctly");
        assertEq(
        ...
        assertEq(
            address(auction.config.unsoldTokensRecipient), UNSOLD_RECIPIENT, "Unsold tokens recipient not set correctly"
        );
        assertEq(auction.config.startTime, block.timestamp, "Start time not set correctly");
        assertEq(auction.config.endTime, block.timestamp + AUCTION_DURATION, "End time not set correctly");
        assertEq(
            auction.state.remainingTokens, TOTAL_TOKENS - KICKER_REWARD_AMOUNT, "Remaining tokens not set correctly"
        );
        assertEq(auction.state.isFinalized, false, "Auction should not be finalized");
    }

    function testPriceDecayLinear() public {
        uint256 auctionId = _createAuction(auctionToken1, DecayFunctions.DecayType.Linear);
        ...
        uint256[] memory expectedPrices = getExpectedPrices(json, keys, "linear");
        for (uint256 i = 0; i < keys.length; i++) {
            vm.warp(initialBlockTimestamp + timeIntervals[i]);

            uint256 currentPrice = auctionManager.getCurrentPrice(auctionId);
            assertEq(currentPrice, expectedPrices[i], "Linear price incorrect at interval");
        }
    }

    function testPriceDecayExponential() public {
        uint256 auctionId = _createAuction(auctionToken1, DecayFunctions.DecayType.Exponential);
        string memory json = vm.readFile(EXPECTED_PRICES_PATH);
        ...
        uint256 initialBlockTimestamp = block.timestamp;
        uint256[] memory timeIntervals = getIntervals(keys);
        uint256[] memory expectedPrices = getExpectedPrices(json, keys, "exponential");
        for (uint256 i = 0; i < keys.length; i++) {
            vm.warp(initialBlockTimestamp + timeIntervals[i]);

            uint256 currentPrice = auctionManager.getCurrentPrice(auctionId);
            assertEq(currentPrice, expectedPrices[i], "Exponential price incorrect at interval");
        }
    }
    ...
    function testClaimedRewards() public {
        uint256 kickerRewardAmount = TOTAL_TOKENS.applyPercentage(KICKER_REWARD_PERCENTAGE);
        ...
        assertEq(creatorBalanceAfter - creatorBalanceBefore, kickerRewardAmount, "Kicker reward not claimed correctly");
    }

    function testMultipleAuctions() public {
        uint256 auctionId1 = _createAuction(auctionToken1, DecayFunctions.DecayType.Linear);
        uint256 auctionId2 = _createAuction(auctionToken2, DecayFunctions.DecayType.Exponential);
        ...
        vm.stopPrank();
        assertEq(auctionId2, auctionId1 + 1, "Auction IDs should be sequential");
        assertEq(auctionToken1.balanceOf(BUYER1), buyAmount, "Buyer should have received tokens from auction 1");
        assertEq(auctionToken2.balanceOf(BUYER1), buyAmount, "Buyer should have received tokens from auction 2");
    }
    ...
    function testMultipleBuyersSameBlock() public {
        ...
        assertEq(auctionToken1.balanceOf(BUYER1), buyAmount, "Buyer1 should have received tokens");
        assertEq(auctionToken1.balanceOf(BUYER2), buyAmount, "Buyer2 should have received tokens");
        assertEq(auctionToken1.balanceOf(BUYER3), buyAmount, "Buyer3 should have received tokens");
    }
    ...
    function testPriceDecreaseOverTime() public {
        ...
        assertGt(halfPrice, threeQuarterPrice, "Price should decrease over time");
        assertGt(threeQuarterPrice, finalPrice, "Price should decrease over time");
    ...
    function testBuyingAllTokens() public {
        ...
        uint256 kickerReward = TOTAL_TOKENS.applyPercentage(KICKER_REWARD_PERCENTAGE);
        ...
        // Check balances
        assertEq(auctionToken1.balanceOf(BUYER1), availableTokens / 2, "Buyer1 balance incorrect");
        assertEq(auctionToken1.balanceOf(BUYER2), availableTokens / 4, "Buyer2 balance incorrect");
        assertEq(auctionToken1.balanceOf(BUYER3), availableTokens / 4, "Buyer3 balance incorrect");
        assertEq(
            auctionToken1.balanceOf(AUCTION_KICKER), kickerReward, "Auction creator should have received kicker reward"
        );
        assertEq(auctionToken1.balanceOf(UNSOLD_RECIPIENT), 0, "Unsold recipient should have no tokens");
    }
    ...
    function testBuyingAllTokensAndFinalizing() public {
        ...
        assertEq(auctionToken1.balanceOf(BUYER1), availableTokens / 2, "Buyer1 balance incorrect");
        assertEq(auctionToken1.balanceOf(BUYER2), availableTokens / 4, "Buyer2 balance incorrect");
        assertEq(auctionToken1.balanceOf(BUYER3), (availableTokens / 4) - 1, "Buyer3 balance incorrect");
        assertEq(
            auctionToken1.balanceOf(AUCTION_KICKER), kickerReward, "Auction creator should have received kicker reward"
        );
        assertEq(auctionToken1.balanceOf(UNSOLD_RECIPIENT), 1, "Unsold recipient should have no tokens");
    }
    ...
    function _createAuction(ERC20Mock _auctionToken, DecayFunctions.DecayType _decayType) internal returns (uint256) {
        vm.prank(AUCTION_KICKER);
        return auctionManager.createAuction(
    ...
    function _testAuctionLifecycle(uint256 _auctionId, bool _isLinearDecay) internal {
        // Test initial state
        uint256 currentPrice = auctionManager.getCurrentPrice(_auctionId);
        assertEq(currentPrice, START_PRICE, "Initial price should be start price");

        // Test price halfway through auction
        vm.warp(block.timestamp + AUCTION_DURATION / 2);
        currentPrice = auctionManager.getCurrentPrice(_auctionId);
        uint256 expectedMidPrice =
            _isLinearDecay ? (START_PRICE + END_PRICE) / 2 : END_PRICE + ((START_PRICE - END_PRICE) * 1 ** 2) / 4;
        assertApproxEqAbs(currentPrice, expectedMidPrice, 1 ether, "Mid-auction price incorrect");

        // Test buying tokens
        uint256 buyAmount = 100 ether;
        vm.prank(BUYER1);
        auctionManager.buyTokens(_auctionId, buyAmount);
        assertEq(auctionToken1.balanceOf(BUYER1), buyAmount, "Buyer should have received tokens");
        ...
        uint256 unsoldAmount = TOTAL_TOKENS - buyAmount - TOTAL_TOKENS.applyPercentage(KICKER_REWARD_PERCENTAGE);
        assertEq(auctionToken1.balanceOf(UNSOLD_RECIPIENT), unsoldAmount, "Unsold tokens not sent to recipient");
    }
     ...
     */
    function getIntervals(string[] memory keys) internal pure returns (uint256[] memory) {
