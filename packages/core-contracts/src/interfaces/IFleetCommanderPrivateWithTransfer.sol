...
/**
 * @title IFleetCommanderPrivateWithTransfer Interface
 * @notice Interface for the FleetCommander contract, which manages asset allocation across multiple Arks.
 * @dev Implementations should gate state-changing user calls using the `Operator` role. They can also
 *      gate the transfer of assets and shares through the whitelist utility.
 */
interface IFleetCommanderPrivateWithTransfer is
    IERC4626,
