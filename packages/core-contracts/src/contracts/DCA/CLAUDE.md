...
- 2026-06-29 — claude — checkUpkeep funding gate now also checks the owner's standard ERC20 approval to PERMIT2 (`sourceVault.allowance(owner, PERMIT2) >= tradeAmount`), the last static precondition it was missing: the Permit2 sub-allowance can be live while the ERC20 approval is revoked/finite, so the pull (`PERMIT2.transferFrom` → `sourceVault.transferFrom` with PERMIT2 as spender) would deterministically revert. Surfaced by the Codex second-opinion review; completes the "checkUpkeep-true ⇒ no static-precondition revert" invariant. View-only; no ABI/struct change. Test `test_CheckUpkeep_ReturnsFalseWhenPermit2Erc20ApprovalInsufficient` (145 DCA tests pass).
- 2026-06-29 — claude — DRY the active-fleet check: new non-reverting `HarborCommandConsumer._isActiveFleetCommander(vault)` is now the single `HARBOR_COMMAND.activeFleetCommanders` read; `_requireActiveFleetCommander` (revert form, used by `executeStrategy` + the `onlyActiveFleetCommander` modifier) and `checkUpkeep` (branch form) both build on it. Behavior-identical; no ABI/struct/commitment change. 144 DCA tests pass.
- 2026-06-29 — claude — `checkUpkeep` now mirrors `executeStrategy`'s static preconditions for the non-terminal trade path (reliability/pre-audit, not an audit finding): after the funding/transfers gates it checks both fleets are active in HarborCommand and computes the oracle/`minOut` path, returning false when `minOut` rounds to zero. Invariant: if checkUpkeep returns true for a non-terminal trade, executeStrategy won't deterministically revert on a static precondition. Terminal checks stay first, so a strategy still finalizes as COMPLETED even with a deregistered vault. A stale/invalid oracle makes checkUpkeep REVERT (never returns true on a bad read; keeper treats it as skip) — documented, not graceful-false, since the oracle read is an internal lib call that can't be try/caught. View-only; no ABI/struct/commitment change. Tests: `test_CheckUpkeep_{ReturnsFalseWhenSourceFleetDecommissioned,ReturnsFalseWhenTargetFleetDecommissioned,ReturnsFalseWhenMinOutRoundsToZero,TerminalCompletionStillReturnsTrueEvenIfFleetDecommissioned,RevertsOnStaleOracle}`.
- 2026-06-29 — claude — refactor (post-codex review): extracted the active-fleet check into `HarborCommandConsumer._requireActiveFleetCommander(vault, label)`; the `onlyActiveFleetCommander` modifier now delegates to it and `executeStrategy` calls it directly, replacing F9's two inline check blocks. Behavior-identical (same `InactiveFleetCommander(vault, label)` revert); no ABI/struct/commitment change. 139 DCA tests pass.
- 2026-06-29 — claude — audit follow-ups (Run #649 + Sherlock): F1 minOut==0 guard (reuse ZeroExpectedOutShares); F3 idempotent edit no longer reverts DuplicateStrategy; F4 editStrategy clamps nextTriggerAt to block.timestamp; F7 resumeStrategy auto-completes terminal strategies; F8/F10 checkUpkeep evaluates terminal conditions before the cadence gate and returns true so the keeper can finalize (keeper sweep tracked separately); F9 executeStrategy runs the terminal short-circuit before the now-inline active-fleet checks (completes even if a vault was deregistered); F11 new TradeAmountTooLarge error rejects tradeAmount > uint160 max. No function-signature/struct/commitment change; F11 adds one error (`TradeAmountTooLarge`) to the ABI — FE + subgraph ABIs regenerated in lockstep; checkUpkeep terminal return flipped false→true (behavioral). Tests in DCAsStrategyManager.t.sol.
