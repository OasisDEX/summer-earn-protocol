...
contract SyrupArkV2USDTTestFork is Test, IArkEvents, ArkTestBase {
    ...
    function test_RequestPartialRedeem_Syrup_fork() public {
        ...
        uint256 sharesAmount = ISyrupPool(SYRUP_USDT_POOL_ADDRESS).convertToExitShares(redeemAmount);
        vm.expectCall(
    ...
    function test_WithdrawableTotalAssets_Syrup_fork() public {
        ...
        emit Disembarked(address(keeper), USDT_ADDRESS, redeemAmount);

        vm.prank(keeper);
        ark.sweep();

        assertEq(IERC20(USDT_ADDRESS).balanceOf(bufferArk), bufferArkUsdtBalanceBefore + redeemAmount);
    }
