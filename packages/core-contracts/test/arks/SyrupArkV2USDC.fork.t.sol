...
contract SyrupArkV2USDCForkTest is Test, IArkEvents, ArkTestBase {
    ...
    function test_RequestPartialRedeem_Syrup_fork() public {
        ...
        uint256 sharesAmount = ISyrupPool(SYRUP_USDC_POOL_ADDRESS)
            .convertToExitShares(withdrawalAmount);
        vm.expectCall(
            address(syrupPool),
            abi.encodeWithSelector(
    ...
    function test_WithdrawableTotalAssets_Syrup_fork() public {
        ...
        emit Disembarked(address(keeper), USDC_ADDRESS, withdrawalAmount);
    ...
    function test_WithdrawableTotalAssets_Syrup_maxuint_fork() public {
        ...
        emit Disembarked(address(keeper), USDC_ADDRESS, 0);
        ...
        assertGt(
            IERC20(USDC_ADDRESS).balanceOf(bufferArk),
            bufferArkUsdcBalanceBefore + amount
        );
    }
    ...
    function test_WithdrawUsingSwap_NonWhitelistedRouter() public {
        ...
        vm.prank(keeper);
        vm.expectRevert(IArkSwapProvider.RouterNotWhitelisted.selector);
    ...
    function test_CancelWithdrawal_NoRequest_Reverts() public {
        vm.prank(keeper);
        vm.expectRevert(IArkWithWithdrawalRequest.NoWithdrawalToClaim.selector);
        ark.cancelWithdrawal();
    }
    ...
    function _keeperAuthorization() internal {
        ...
        uint256 amount = 3; // 1 USDC
        deal(USDC_ADDRESS, keeper, amount * 2);

        uint256 nonce = ISyrupRouter(SYRUP_USDC_ROUTER_ADDRESS).nonces(
            address(ark)
        );
        uint256 bitmap = 16;
