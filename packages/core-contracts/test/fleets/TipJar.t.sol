...
//    }
    ...
    function test_ShakeWithAccruedInterest() public {
        ...
        uint256 expectedTreasuryAmount = totalAmount - expectedMockRecipientAmount - expectedAnotherRecipientAmount;

        // Check balances
        assertApproxEqRel(underlyingToken.balanceOf(mockTipStreamRecipient), expectedMockRecipientAmount, 1);
        assertApproxEqRel(underlyingToken.balanceOf(anotherMockTipStreamParticipant), expectedAnotherRecipientAmount, 1);
        assertApproxEqRel(underlyingToken.balanceOf(treasury), expectedTreasuryAmount, 1);
    }
//
