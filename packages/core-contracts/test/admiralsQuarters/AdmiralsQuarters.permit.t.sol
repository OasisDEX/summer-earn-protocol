...
contract AdmiralsQuartersPermitTest is AdmiralsQuartersTest {
    ...
    function test_enterFleetWithPermit() public {
        ...
        emit IAdmiralsQuartersEvents.FleetEntered(owner, address(usdcFleet), amount, amount);

        admiralsQuarters.multicall(calls);
        vm.stopPrank();

        // Verify allowance is used up
        assertEq(token.allowance(owner, address(admiralsQuarters)), 0, "Allowance should be used up");
        ...
        assertApproxEqAbs(depositedAssets, amount, 2, "Shares should convert to approx deposited amount");

        assertEq(token.balanceOf(address(admiralsQuarters)), 0, "AdmiralsQuarters should have 0 balance");
    }
    ...
    function test_enterFleetWithPermit2() public {
        ...
        ISignatureTransfer.PermitTransferFrom memory permit = ISignatureTransfer.PermitTransferFrom({
            permitted: ISignatureTransfer.TokenPermissions({ token: IERC20(USDC_ADDRESS), amount: amount }),
            nonce: 0,
            deadline: deadline
        });

        bytes memory signature = _getPermit2Signature(permit, address(admiralsQuarters), ownerPrivateKey);
        ...
        assertEq(IERC20(USDC_ADDRESS).balanceOf(address(admiralsQuarters)), 0, "AdmiralsQuarters should have 0 balance");
    }

    // Helper to get USDC nonce
    function _getUSDCNonce(address token, address user) internal view returns (uint256) {
        (, bytes memory returnData) = token.staticcall(abi.encodeWithSignature("nonces(address)", user));
        return abi.decode(returnData, (uint256));
    }

    // Helper to get USDC digest
    function _getUSDCDigest(address token, bytes32 structHash) internal view returns (bytes32) {
        (, bytes memory returnData) = token.staticcall(abi.encodeWithSignature("DOMAIN_SEPARATOR()"));
