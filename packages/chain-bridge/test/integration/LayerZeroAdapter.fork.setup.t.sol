...
abstract contract LayerZeroAdapterForkSetupTest is Test {
    ...
    address public constant READ_LIB_1002_BASE = 0x1273141a3f7923AA2d9edDfA402440cE075ed8Ff;
    address public constant EXECUTOR_BASE = 0x2CCA08ae69E0C44b18a57Ab2A87644234dAebaE4;
    address public constant READ_DVN_BASE = 0xB1473AC9f58FB27597a21710da9D1071841E8163;
    ...
    function _setupContracts() internal {
        ...
        // Deploy registry
        registry = new CrossChainRegistry(address(accessManager), SOURCE_CHAIN_ID);
        ...
        router = new BridgeRouterTestHelper(address(accessManager), address(registry));

        vm.startPrank(governor);

        // Initialize bridge configuration
        registry.initializeBridgeConfiguration(address(router), DEFAULT_GAS_LIMIT);
        vm.stopPrank();
    ...
    function _configureAdapter() internal {
        ...
        readDVNs[0] = readDVN;
        layerZeroAdapter.configureReadDVNs(readLib1002, readDVNs, confirmations, executor);

        // Step 5: Set up peer for cross-chain communication to Arbitrum
        bytes32 peerAddressBytes32 = bytes32(uint256(uint160(address(layerZeroAdapter))));
        ...
        layerZeroAdapter.setPeer(READ_CHANNEL_THRESHOLD + 1, peerAddressBytes32);
    ...
    function _verifyAdapterConfiguration() internal view {
        ...
        assertTrue(router.isValidAdapter(address(layerZeroAdapter)), "Adapter not registered with router");

        // Test LayerZero EID mapping
        assertEq(layerZeroAdapter.chainToLzEid(DEST_CHAIN_ID), ARB_LZ_EID, "Chain to LZ EID mapping incorrect");
        assertEq(layerZeroAdapter.lzEidToChain(ARB_LZ_EID), DEST_CHAIN_ID, "LZ EID to chain mapping incorrect");

        // Test read channel configuration
        assertEq(layerZeroAdapter.readChannelId(), READ_CHANNEL_ID, "Read channel ID not configured");

        // Test CrossChainRegistry integration
        assertEq(layerZeroAdapter.bridgeRouter(), address(router), "Bridge router not accessible through registry");
    }

    // Helper function to deploy a fresh unconfigured layerZeroAdapter for negative tests
    function _deployUnconfiguredAdapter() internal returns (LayerZeroAdapterTestHelper) {
    ...
    function _setOperationMapping(bytes32 guid, bytes32 operationId) internal {
        ...
        assertEq(
            layerZeroAdapter.lzMessageToOperationId(guid), operationId, "Operation mapping should be set correctly"
        );
    }
