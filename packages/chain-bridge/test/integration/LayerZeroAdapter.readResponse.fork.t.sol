 ...
 */
contract LayerZeroAdapterReadResponseBaseForkTest is LayerZeroAdapterForkSetupTest {
    ...
    function testSuccessfulReadResponseHandling() public {
        ...
        // Setup test data
        bytes32 operationId = keccak256(abi.encodePacked("test_read_operation_1"));
    ...
    function testReadResponseWithUnknownOperationId() public {
        ...
        bytes memory responseData = abi.encode(uint256(67890), "unknown_response");
        ...
        layerZeroAdapter.lzReceive(origin, unknownGuid, responseData, address(0), "");
    ...
    function testReadResponseDeliveryFailure() public {
        ...
        // Setup test data
        bytes32 operationId = keccak256(abi.encodePacked("test_read_operation_2"));
    ...
    function testReadChannelThresholdBoundaryConditions() public pure {
        console.log("=== Testing Read Channel Threshold Boundary Conditions ===");
        ...
        assertTrue(READ_CHANNEL_THRESHOLD < READ_CHANNEL_ID, "Read channel ID should be above threshold");
        ...
        assertTrue(atThreshold < aboveThreshold, "Above threshold should be greater than at threshold");

        assertTrue(aboveThreshold > READ_CHANNEL_THRESHOLD, "Above threshold should trigger read response handling");
    ...
    function testLayerZeroEndpointIntegration() public view {
        ...
        console.log("Testing read response handling with real LayerZero endpoint configuration");
        ...
        assertTrue(READ_LIB_1002_BASE.code.length > 0, "ReadLib1002 should have code");
        ...
        console.log("[SUCCESS] LayerZero configuration verified for read responses");
    }
    ...
    function testMultipleReadResponsesHandling() public {
        ...
        for (uint256 i = 0; i < numResponses; i++) {
            ...
            router.setOperationStatus(operationIds[i], BridgeTypes.OperationStatus.SENT);

            // Associate the operation with the layerZeroAdapter (required for authorization)
            router.setOperationToAdapter(operationIds[i], address(layerZeroAdapter));
        ...
        for (uint256 i = 0; i < numResponses; i++) {
            bytes memory responseData = abi.encode(uint256(i * 1000), string(abi.encodePacked("response_", i)));
            ...
            vm.prank(LZ_ENDPOINT_BASE);
            layerZeroAdapter.lzReceive(origin, guids[i], responseData, address(0), "");
        }
