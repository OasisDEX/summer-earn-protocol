...
contract MockLayerZeroEndpoint {
    ...
    event ComposeSent(address indexed from, address indexed to, bytes32 indexed guid, uint16 index, bytes message);
    ...
    function sendCompose(address _to, bytes32 _guid, uint16 _index, bytes calldata _message) external {
        ...
        composeCalls.push(ComposeCall({ to: _to, guid: _guid, index: _index, message: _message }));
     ...
     */
    function executeCompose(uint256 composeIndex, address executor, bytes calldata extraData) external {
        require(composeIndex < composeCalls.length, "Invalid compose index");

        ComposeCall memory call = composeCalls[composeIndex];
        bytes32 composeKey = keccak256(abi.encode(call.to, call.guid, call.index));
     ...
     */
    function getComposeCall(uint256 index)
        external
        view
        returns (address to, bytes32 guid, uint16 composeIndex, bytes memory message)
    {
