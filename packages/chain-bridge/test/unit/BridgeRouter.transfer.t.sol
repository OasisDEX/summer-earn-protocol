...
contract BridgeRouterTransferTest is BridgeRouterSetup {
    ...
    function testRefundsExcessFeeOnTransfer() public {
        vm.startPrank(keeper);
        ...
        uint256 adapterBefore = address(mockAdapter).balance;
        uint256 routerBefore = address(router).balance;

        uint256 overpay = nativeFee + 0.05 ether;
        router.executeTransferAssets{value: overpay}(
            BridgeTypes.ExecuteTransferParams({
                destinationChainId: DEST_CHAIN_ID,
                asset: address(token),
                amount: TRANSFER_AMOUNT,
        ...
        uint256 adapterAfter = address(mockAdapter).balance;
        uint256 routerAfter = address(router).balance;

        assertEq(
            adapterAfter - adapterBefore,
            nativeFee,
            "adapter must receive buffered fee"
        );
        assertEq(routerAfter, routerBefore, "router must not retain excess");
        vm.stopPrank();
    }

    function testRefundsExcessFeeOnReadState() public {
        vm.startPrank(keeper);
    ...
    function testRefundsExcessFeeOnSendMessage() public {
        vm.startPrank(keeper);
        ...
        uint256 overpay = nativeFee + 0.05 ether;
        router.executeSendMessage{value: overpay}(
