...
contract CrossChainRegistryIntegrationTest is Test {
    ...
    function test_endToEndRegistrationAndValidation() public {
        ...
        emit CrossChainRelationshipRegistered(
            arkAddress, proxyAddress, CURRENT_CHAIN_ID, TARGET_CHAIN_ID, ARK_FLEET_RELATIONSHIP
        );
        ...
        (address targetContract, uint16 chainId) = registry.getTargetForSource(arkAddress, ARK_FLEET_RELATIONSHIP);
        assertEq(targetContract, proxyAddress);
        assertEq(chainId, TARGET_CHAIN_ID);

        address sourceContract =
            registry.getSourceForTarget(CURRENT_CHAIN_ID, TARGET_CHAIN_ID, proxyAddress, ARK_FLEET_RELATIONSHIP);
    ...
    function test_registrationWithDifferentChainIds() public {
        ...
        vm.prank(governor);
        registry.registerCrossChainRelationship(
            arkAddress, proxyAddress, differentSourceChain, differentTargetChain, ARK_FLEET_RELATIONSHIP
        );
    ...
    function test_multipleRelationshipTypes() public {
        ...
        vm.prank(governor);
        registry.registerCrossChainRelationship(arkAddress, proxy1, CURRENT_CHAIN_ID, TARGET_CHAIN_ID, arkFleetType);

        vm.prank(governor);
        registry.registerCrossChainRelationship(arkAddress, proxy2, CURRENT_CHAIN_ID, TARGET_CHAIN_ID, bridgeType);

        // Verify both relationships exist
        (address target1, uint16 chain1) = registry.getTargetForSource(arkAddress, arkFleetType);
        assertEq(target1, proxy1);
        assertEq(chain1, TARGET_CHAIN_ID);

        (address target2, uint16 chain2) = registry.getTargetForSource(arkAddress, bridgeType);
        assertEq(target2, proxy2);
        assertEq(chain2, TARGET_CHAIN_ID);

        // Check both are valid
        assertTrue(registry.isValidCrossChainPair(arkAddress, proxy1, CURRENT_CHAIN_ID, TARGET_CHAIN_ID, arkFleetType));
        assertTrue(registry.isValidCrossChainPair(arkAddress, proxy2, CURRENT_CHAIN_ID, TARGET_CHAIN_ID, bridgeType));
    ...
    function test_unregistrationCleansUpCorrectly() public {
        ...
        (address targetContract, uint16 chainId) = registry.getTargetForSource(ark2, ARK_FLEET_RELATIONSHIP);
        assertEq(targetContract, proxy2);
        assertEq(chainId, TARGET_CHAIN_ID);

        address sourceContract =
            registry.getSourceForTarget(CURRENT_CHAIN_ID, TARGET_CHAIN_ID, proxy2, ARK_FLEET_RELATIONSHIP);
    ...
    function test_relationshipTypesAreIndependent() public {
        ...
        vm.prank(governor);
        registry.registerCrossChainRelationship(arkAddress, proxyAddress, CURRENT_CHAIN_ID, TARGET_CHAIN_ID, type1);

        vm.prank(governor);
        registry.registerCrossChainRelationship(arkAddress, proxyAddress, CURRENT_CHAIN_ID, TARGET_CHAIN_ID, type2);
        ...
        assertTrue(registry.isValidCrossChainPair(arkAddress, proxyAddress, CURRENT_CHAIN_ID, TARGET_CHAIN_ID, type1));
        assertTrue(registry.isValidCrossChainPair(arkAddress, proxyAddress, CURRENT_CHAIN_ID, TARGET_CHAIN_ID, type2));

        // Unregister one type
        vm.prank(governor);
        registry.unregisterCrossChainRelationship(arkAddress, type1, TARGET_CHAIN_ID);
    ...
    function test_supportedRelationshipTypes() public {
        ...
        vm.prank(governor);
        registry.registerCrossChainRelationship(arkAddress, proxyAddress, CURRENT_CHAIN_ID, TARGET_CHAIN_ID, newType);
