...
export function ConnectButton() {
  ...
  if (!isConnected) {
    return (
      <button
        onClick={onOpen}
        className="bg-primary hover:bg-primary/90 text-on-primary px-4 py-2 rounded-lg text-sm font-semibold transition-all shadow-lg shadow-primary/20"
      >
        Connect
      </button>
    )
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      >
        ...
        <span className="text-sm font-medium text-on-surface">{short}</span>
      ...
      <button
        ...
        className="px-2 py-1.5 rounded-lg bg-error/20 text-error hover:bg-error/30 text-xs font-medium transition-colors"
