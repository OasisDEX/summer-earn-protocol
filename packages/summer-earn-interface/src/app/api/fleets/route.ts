    ...
    const results = await Promise.all(
      allFleets.map(async (fleetAddress) => {
        const [name, symbol, assetAddress, totalAssets, withdrawableTotalAssets] =
          await Promise.all([
            client.readContract({ address: fleetAddress, abi: fleetCommanderAbi, functionName: 'name' }),
            client.readContract({ address: fleetAddress, abi: fleetCommanderAbi, functionName: 'symbol' }),
            client.readContract({ address: fleetAddress, abi: fleetCommanderAbi, functionName: 'asset' }),
            client.readContract({ address: fleetAddress, abi: fleetCommanderAbi, functionName: 'totalAssets' }),
            client.readContract({ address: fleetAddress, abi: fleetCommanderAbi, functionName: 'withdrawableTotalAssets' }),
          ])
