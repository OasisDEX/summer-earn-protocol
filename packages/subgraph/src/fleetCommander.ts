...
function createDepositEventEntity(
  event: DepositEvent,
...
function createWithdrawEventEntity(
  event: WithdrawEvent,
  normalizedAmountUSD: BigDecimal,
  account: Account,
  vault: Vault,
...
function updateVault(
  ...
  vault.totalValueLockedUSD = formatAmount(
    totalAssets,
    BigInt.fromI32(depositToken.decimals),
  ).times(priceInUSD.price)
