...
export class EnsRegistry extends ethereum.SmartContract {
  ...
  try_setSubnodeOwner(node: Bytes, label: Bytes, owner: Address): ethereum.CallResult<Bytes> {
  ...
  ttl(node: Bytes): BigInt {
    let result = super.call('ttl', 'ttl(bytes32):(uint64)', [ethereum.Value.fromFixedBytes(node)])
