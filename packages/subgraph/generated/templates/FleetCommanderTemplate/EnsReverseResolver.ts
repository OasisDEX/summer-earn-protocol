...
export class EnsReverseResolver extends ethereum.SmartContract {
  static bind(address: Address): EnsReverseResolver {
    return new EnsReverseResolver('EnsReverseResolver', address)
  }
  ...
  claimForAddr(addr: Address, owner: Address, resolver: Address): Bytes {
    let result = super.call('claimForAddr', 'claimForAddr(address,address,address):(bytes32)', [
  ...
  try_claimForAddr(addr: Address, owner: Address, resolver: Address): ethereum.CallResult<Bytes> {
    let result = super.tryCall('claimForAddr', 'claimForAddr(address,address,address):(bytes32)', [
      ethereum.Value.fromAddress(addr),
      ethereum.Value.fromAddress(owner),
      ethereum.Value.fromAddress(resolver),
    ])
  ...
  claimWithResolver(owner: Address, resolver: Address): Bytes {
    let result = super.call('claimWithResolver', 'claimWithResolver(address,address):(bytes32)', [
      ethereum.Value.fromAddress(owner),
      ethereum.Value.fromAddress(resolver),
    ])
  ...
  try_claimWithResolver(owner: Address, resolver: Address): ethereum.CallResult<Bytes> {
  ...
  defaultResolver(): Address {
    let result = super.call('defaultResolver', 'defaultResolver():(address)', [])

    return result[0].toAddress()
  }

  try_defaultResolver(): ethereum.CallResult<Address> {
    let result = super.tryCall('defaultResolver', 'defaultResolver():(address)', [])
    if (result.reverted) {
      return new ethereum.CallResult()
    }
    let value = result.value
  ...
  node(addr: Address): Bytes {
    let result = super.call('node', 'node(address):(bytes32)', [ethereum.Value.fromAddress(addr)])
  ...
  setNameForAddr(addr: Address, owner: Address, resolver: Address, name: string): Bytes {
