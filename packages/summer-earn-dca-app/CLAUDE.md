...
- 2026-06-30 — claude — broad app↔contract drift audit against the #875 contract: `StrategyConfigTuple`/`commitment.ts`/`encode.ts`/`types`, the `as const` ABI, write-call signatures, and `subgraph/{queries,types}.ts` all verified already in sync (no change). The only drift was `FRIENDLY_REVERT_LABELS` in `useTxToast.ts` — completed the revert decoder with the missing user-facing errors: `TradeAmountTooLarge` (supersedes the 2026-06-29 "did NOT add" note), `ZeroTradeAmount`, `ZeroMaxTrades`, `InvalidSlippage`, `InvalidOwner`, `SameAsset`, `IntervalTooShort`, `IntervalTooLong`, `InvalidFeedAddress`, `UnauthorizedOwner`, `InactiveFleetCommander`, `ZeroDeposit`, `DepositSharesBelowMin`, `PriceAboveCeiling`, `PriceBelowFloor`, `ExecutionWindowNotReached`, `SwapOutputBelowMinOut`. UX-only (toast text); `tsc --noEmit` clean. The DCA subgraph was audited in the same pass (event signatures, 14-field tuple + nested feeds, ExecutionCompleted params, schema, ABI, status enum, mappings) and confirmed byte-for-byte in sync — no subgraph change.
