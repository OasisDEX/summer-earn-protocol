...
import { getPublicClient } from '../../summer-earn-gov-validator/src/config/rpc'
...
export class Poller {
  ...
  async poll(targetChatId?: string | number) {
    for (const network of Object.values(SupportedNetworks)) {
      try {
        ...
        const processor = new EventProcessor(this.notifier, network, client, targetChatId)
  ...
  async processSingleTransaction(network: SupportedNetworks, txHash: `0x${string}`, targetChatId?: string | number) {
    try {
      const chain = viemChains[network]
      const client = getPublicClient(chain.id)
