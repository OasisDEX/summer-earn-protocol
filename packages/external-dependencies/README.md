...
Every Solidity package in the repo. Forge remappings (in each package's `remappings.txt`) resolve
`forge-std/`, `@openzeppelin/contracts/`, `morpho-blue/`, `metamorpho/`, `@prb/math/`,
`@pendle/core-v2/`, `solmate/`, etc. to paths under `node_modules/@summerfi/dependencies/lib/`.
