...
async function deployGov() {
  ...
  const config: BaseConfig = getConfigByNetwork(
    hre.network.name,
    { gov: false },
    useBummerConfig,
  ) as BaseConfig
    ...
    try {
      ...
      switch (step) {
        ...
        case STEPS.VERIFY:
          ...
          break
        case STEPS.ROLES:
          await rolesGovV2(useBummerConfig)
          break
        case STEPS.PEER:
          ...
          if (confirmPeering) {
            await peerGovV2(useBummerConfig)
          } else {
        ...
        case STEPS.FINALIZE:
          ...
          if (confirmed) {
            ...
            await finalizeGovV2(governorAddressesToRevoke, useBummerConfig)
          } else {
