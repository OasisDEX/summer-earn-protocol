...
export async function addTokenWhitelistSafe(useBummerConfig = false) {
  ...
  const summerToken = await hre.viem.getContractAt('SummerToken' as string, tokenAddress)
  ...
  // Note: Assuming addToWhitelist function exists based on usage in other scripts
  // If the function doesn't exist, it may need to be added to the contract
  ...
  console.log(
    kleur.cyan(
      `\nNote: If addToWhitelist function doesn't exist on SummerToken, it may need to be added to the contract first.`,
    ),
  )
}
