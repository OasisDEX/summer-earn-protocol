...
export async function addVestingFactoriesSafe(useBummerConfig = false) {
  ...
  const escrow = await hre.viem.getContractAt('SummerVestingWalletsEscrow' as string, escrowAddress)
  ...
  const vestingFactories = [
    ...
    config.deployedContracts.gov.summerVestingFactoryV2?.address ||
      config.deployedContracts.govV2.summerVestingFactoryV2?.address ||
      '0x3aA85a023C0e935CDb5d1CBB2d7BC5EAC5c69BeB',
  ]
  ...
  for (const factoryAddress of vestingFactories) {
    ...
    const isAlreadyAdded = factories.includes(validatedFactory as Address)
