...
async function updateCrossChainConfigPhase2(
  ...
  console.log(
    kleur.cyan(
      '1. Register adapter peers: npx hardhat run scripts/x-chain/post-deployment/register-ark-fleet.ts --network <chain>',
    ),
  )
  console.log(
    kleur.cyan(
      '2. Verify setup: npx hardhat run scripts/x-chain/post-deployment/verify-setup.ts --network <chain>',
    ),
  )
}
