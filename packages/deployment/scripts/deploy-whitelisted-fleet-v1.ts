/**
 * Whitelisted fleet deployment — V1 institution registry variant.
 *
 * Identical to `deploy-whitelisted-fleet.ts` except the pre-flight registration gate checks the
 * legacy `institutionalVaultRegistry` (V1) instead of `institutionalVaultRegistryV2`. Use this for
 * institutions that were onboarded into the V1 registry and have no V2 registry on the target
 * network (e.g. ExtDemoCorp_3 on Base). The registry is only used as an "is this institution
 * registered?" gate; everything the deploy touches (PAM / HarborCommand / AdmiralsQuarters /
 * RoundsVaults) is registry-agnostic, so the rest of the flow is unchanged.
 *
 * Keep this file in sync with `deploy-whitelisted-fleet.ts`; the only intended difference is the
 * registry gate below.
 */
...
async function main() {
  ...
  // Gate by InstitutionalVaultRegistry (V1): institution must be registered before continuing.
  // This is the only difference from deploy-whitelisted-fleet.ts, which gates on the V2 registry.
