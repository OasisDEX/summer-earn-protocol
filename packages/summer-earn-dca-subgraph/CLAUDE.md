...
- 2026-06-30 — claude — v5 address swap (bd aphelion-app-4z5 done): `config/{base,mainnet}.json` now point at the audited v5 managers (Base `0x659d…952a` @ startBlock 48016006, mainnet `0xa0d3…985D` @ startBlock 25430408; feed-start-blocks ~14d prior), superseding the interim-v4 values from the entry below. `build:base` + `build:mainnet` clean. Version stays 0.2.0 (not yet deployed).
