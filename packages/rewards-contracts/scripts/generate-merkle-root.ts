...
async function selectChainAndFile(): Promise<{ chainId: string; filename: string } | null> {
  try {
    ...
    const chainDirs = fs
      ...
      .filter((file) => fs.statSync(path.join(distributionsDir, file)).isDirectory())
    ...
    const chainDir = path.join(distributionsDir, chainResponse.chainId)
    ...
    if (distributionFiles.length === 0) {
      console.error(`No distribution files found in distributions/${chainResponse.chainId}/`)
      return null
    }
...
async function processDistributionFile(chainId: string, filename: string) {
  try {
    ...
    const filePath = path.join(__dirname, `../distributions/${chainId}/${filename}`)
    ...
    const outputDir = path.join(__dirname, '../merkle-trees', chainId)
