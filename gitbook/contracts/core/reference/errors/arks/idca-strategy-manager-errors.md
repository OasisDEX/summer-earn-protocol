---
description: >-
  Custom errors raised by the DCA (dollar-cost-averaging) strategy manager
---
...
Reverts when the slippage-adjusted minimum output `minOut` is zero —
...
### TradeAmountTooLarge
Reverts when `config.tradeAmount` exceeds `type(uint160).max`, the
