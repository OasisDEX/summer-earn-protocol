...
<!-- AUTOGENERATED by forge doc + scripts/docs/clean-forge-docs.mjs — do not edit. Edit the NatSpec in packages/core-contracts/src/contracts/FleetCommanderConfigProviderWhitelist.sol instead. -->

# FleetCommanderConfigProviderWhitelist
[Git Source](https://github.com/OasisDEX/summer-earn-protocol/blob/main/packages/core-contracts/src/contracts/FleetCommanderConfigProviderWhitelist.sol)

**Inherits:**
ProtocolAccessManagedV2, [FleetCommanderPausable](./fleet-commander-pausable.md), ConfigurationManaged, [IFleetCommanderConfigProviderWhitelist](../interfaces/i-fleet-commander-config-provider-whitelist.md), [Whitelist](../utils/Whitelist/whitelist.md)
...
**Note:**
see: IFleetCommanderConfigProviderWhitelist


## State Variables
### config
The mutable configuration set for this Whitelist fleet
...
### details
A string containing additional details about this fleet
...
### _activeArks
Internal set of active arks (excluding the buffer ark)
...
The maximum number of rebalance operations allowed in a single call
...
The default minimum pause time for governance actions
...
### transfersEnabled
If true, whitelist-restricted transfers are enabled for this fleet's tokens
...
Wires this Whitelist-enabled fleet configuration provider together with its
...
Inherits `ProtocolAccessManagedV2` (access control), `FleetCommanderPausable`
...
|----|----|-----------|
|`params`|`FleetCommanderWhitelistParams`|The aggregated initialization parameters; see {FleetCommanderWhitelistParams}.|
...
```solidity
function getConfig() external view override returns (FleetConfigWhitelist memory);
```
...
Sets the maximum number of allowed rebalance operations.
...
```solidity
function setFleetTokenTransferability(bool newStatus) external onlyGovernor whenNotPaused;
```
**Parameters**

|Name|Type|Description|
|----|----|-----------|
|`newStatus`|`bool`|The new status of the fleet token transferability|


### setOperatorGatewayStatus

Sets the operator gateway status

Entry (deposit/mint) and exit (withdraw/redeem) operations are gated by the operator
...
```solidity
function setOperatorGatewayStatus(bool newStatus) external onlyGovernor whenNotPaused;
```
**Parameters**
