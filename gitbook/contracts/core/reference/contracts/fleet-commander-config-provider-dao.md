---
description: >-
  This contract provides configuration management for the FleetCommanderConfigProviderDao
---

<!-- AUTOGENERATED by forge doc + scripts/docs/clean-forge-docs.mjs — do not edit. Edit the NatSpec in packages/core-contracts/src/contracts/FleetCommanderConfigProviderDao.sol instead. -->

# FleetCommanderConfigProviderDao
[Git Source](https://github.com/OasisDEX/summer-earn-protocol/blob/main/packages/core-contracts/src/contracts/FleetCommanderConfigProviderDao.sol)

**Inherits:**
ProtocolAccessManaged, [FleetCommanderPausable](./fleet-commander-pausable.md), ConfigurationManaged, [IFleetCommanderConfigProviderDao](../interfaces/i-fleet-commander-config-provider-dao.md)
...
**Note:**
see: IFleetCommanderConfigProviderDao
...
Returns the DAO fleet's tip jar

Overrides ConfigurationManaged.tipJar() so DAO fleets don't accidentally use the global tip jar.
...
```solidity
function getConfig() external view override returns (FleetConfigDao memory);
```
...
```solidity
function setArkDepositCapToZero(address ark) external onlyGuardian onlyActiveArk(ark) whenNotPaused;
```
**Parameters**
...
```solidity
function setTipJar(address newTipJar) external onlyCurator(address(this)) whenNotPaused;
```
**Parameters**
