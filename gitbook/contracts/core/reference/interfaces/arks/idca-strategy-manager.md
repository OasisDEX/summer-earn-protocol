---
description: >-
  Interface for the manager that creates and keeper-executes dollar-cost-averaging strategies between
---

<!-- AUTOGENERATED by forge doc + scripts/docs/clean-forge-docs.mjs — do not edit. Edit the NatSpec in packages/core-contracts/src/interfaces/arks/IDCAStrategyManager.sol instead. -->

# IDCAStrategyManager
[Git Source](https://github.com/OasisDEX/summer-earn-protocol/blob/main/packages/core-contracts/src/interfaces/arks/IDCAStrategyManager.sol)
...
```solidity
function createStrategy(StrategyConfig calldata config) external returns (uint256 strategyId);
```
**Parameters**
...
```solidity
function pauseStrategy(uint256 strategyId, StrategyConfig calldata config) external;
```
**Parameters**
...
```solidity
function resumeStrategy(uint256 strategyId, StrategyConfig calldata config) external;
```
**Parameters**
...
```solidity
function cancelStrategy(uint256 strategyId, StrategyConfig calldata config) external;
```
**Parameters**
...
```solidity
function executeStrategy(uint256 strategyId, StrategyConfig calldata config, bytes calldata ensoData) external;
```
**Parameters**
...
Returns the stored commitment hash for a strategy.


```solidity
function strategyCommitments(uint256 strategyId) external view returns (bytes32);
```
**Parameters**
...
|Name|Type|Description|
|----|----|-----------|
|`<none>`|`bytes32`|keccak256(abi.encode(config)) at the time of the last create or edit.|
...
Returns whether a commitment hash is currently registered as active.

Active commitments cannot be reused by `createStrategy` or `editStrategy`
...
|Name|Type|Description|
|----|----|-----------|
|`commitment`|`bytes32`|Hash to query.|

**Returns**

|Name|Type|Description|
|----|----|-----------|
|`<none>`|`bool`|`true` if the hash is registered and has not been freed.|
...
## Structs
### StrategyConfig
Immutable user-defined parameters that define a DCA strategy.

The struct is ABI-encoded and hashed to produce the commitment stored
...
Bundle of the two Permit2 signatures required by
...
### StrategyState
Mutable runtime state of a DCA strategy.
...
## Enums
### Status
Lifecycle states of a DCA strategy.
